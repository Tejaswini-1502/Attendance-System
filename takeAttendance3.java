...
public class takeAttendance3 extends ActivityLogin {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db.addOneDBMS(studentmodel1);
        db.addOneDBMS(studentmodel2);
        db.addOneDBMS(studentmodel3);
        db.addOneDBMS(studentmodel4);
        db.addOneDBMS(studentmodel5);
        db.addOneDBMS(studentmodel6);
        db.addOneDBMS(studentmodel7);
        db.addOneDBMS(studentmodel8);
        db.addOneDBMS(studentmodel9);
        db.addOneDBMS(studentmodel10);
        db.addOneDBMS(studentmodel11);
        db.addOneDBMS(studentmodel12);

        submit.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                db.updateClassHeldDBMS();
                if(ch1.isChecked()){
                    db.updateAttendanceDBMS(1);
                }
                if(ch2.isChecked()){
                    db.updateAttendanceDBMS(2);
                }
                if(ch3.isChecked()){
                    db.updateAttendanceDBMS(3);
                }
                if(ch4.isChecked()){
                    db.updateAttendanceDBMS(4);
                }
                if(ch5.isChecked()){
                    db.updateAttendanceDBMS(5);
                }
                if(ch6.isChecked()){
                    db.updateAttendanceDBMS(6);
                }
                if(ch7.isChecked()){
                    db.updateAttendanceDBMS(7);
                }
                if(ch8.isChecked()){
                    db.updateAttendanceDBMS(8);
                }
                if(ch9.isChecked()){
                    db.updateAttendanceDBMS(9);
                }
                if(ch10.isChecked()){
                    db.updateAttendanceDBMS(10);
                }
                if(ch11.isChecked()){
                    db.updateAttendanceDBMS(11);
                }
                if(ch12.isChecked()){
                    db.updateAttendanceDBMS(12);
                }
            }
        });
